...
const ArrowUpRight = ({ size = 24, strokeWidth = 2, className = "" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className={className}>
    <line x1="7" y1="17" x2="17" y2="7" />
    <polyline points="7 7 17 7 17 17" />
  </svg>
);
const Radio = ({ size = 24, strokeWidth = 2, className = "" }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className={className}>
...
function HeroBlackHole() {
  ...
  useEffect(() => {
    ...
    const photonRing = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xffd9a3, transparent: true, opacity: 0.9 })
    );
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      if (tint < 0.7) {
        starCol[i * 3] = 0.95; starCol[i * 3 + 1] = 0.92; starCol[i * 3 + 2] = 0.82;
      } else if (tint < 0.9) {
        starCol[i * 3] = 0.85; starCol[i * 3 + 1] = 0.68; starCol[i * 3 + 2] = 0.34;
      } else {
        starCol[i * 3] = 0.5; starCol[i * 3 + 1] = 0.78; starCol[i * 3 + 2] = 0.85;
      }
    }
    starGeo.setAttribute("position", new THREE.BufferAttribute(starPos, 3));
    starGeo.setAttribute("color", new THREE.BufferAttribute(starCol, 3));
    const starMat = new THREE.PointsMaterial({ size: 0.08, vertexColors: true, sizeAttenuation: true, transparent: true, opacity: 0.9 });
    ...
    return () => {
      ...
      photonRing.geometry.dispose(); photonRing.material.dispose();
      horizon.geometry.dispose(); horizon.material.dispose();
      if (renderer.domElement.parentNode === mount) mount.removeChild(renderer.domElement);
    };
  }, []);

  // FIX: explicit inline style instead of Tailwind 'absolute inset-0'
  return <div ref={mountRef} style={{ position: 'absolute', top: 0, right: 0, bottom: 0, left: 0 }} />;
}
...
function HawkingRadiation({ progressRef }) {
  ...
  useEffect(() => {
    ...
    const spawnPair = () => {
      ...
      data[n1].age = 0; data[n1].life = 4 + Math.random() * 2;
      data[n1].dir = dir.clone(); data[n1].speed = 0.4 + Math.random() * 0.3; data[n1].escaped = true;
      positions[n1 * 3] = px; positions[n1 * 3 + 1] = py; positions[n1 * 3 + 2] = pz;

      data[n2].age = 0; data[n2].life = 0.6 + Math.random() * 0.3;
      data[n2].dir = dir.clone().multiplyScalar(-1); data[n2].speed = 0.15; data[n2].escaped = false;
      positions[n2 * 3] = px; positions[n2 * 3 + 1] = py; positions[n2 * 3 + 2] = pz;
    };
    ...
    const animate = () => {
      ...
      const spawnRate = 1 + progress * 35;
      lastSpawn += dt;
      const interval = 1 / spawnRate;
      while (lastSpawn > interval) { spawnPair(); lastSpawn -= interval; }

      for (let i = 0; i < MAX; i++) {
        const d = data[i];
        if (d.age >= d.life) { sizes[i] = 0; continue; }
        d.age += dt;
        const cur = new THREE.Vector3(positions[i * 3], positions[i * 3 + 1], positions[i * 3 + 2]);
        cur.add(d.dir.clone().multiplyScalar(d.speed * dt));
        positions[i * 3] = cur.x; positions[i * 3 + 1] = cur.y; positions[i * 3 + 2] = cur.z;
        const lifeT = d.age / d.life;
        const fade = Math.sin(lifeT * Math.PI);
        if (d.escaped) {
          colors[i * 3] = 1.0; colors[i * 3 + 1] = 0.65 + 0.2 * fade; colors[i * 3 + 2] = 0.25 * fade;
          sizes[i] = 4 * fade * (0.6 + progress * 0.6);
        } else {
          colors[i * 3] = 0.45 * fade; colors[i * 3 + 1] = 0.7 * fade; colors[i * 3 + 2] = 1.0;
          sizes[i] = 3 * fade;
        }
      }
    ...
    return () => {
      ...
      bh.geometry.dispose(); bh.material.dispose();
      if (renderer.domElement.parentNode === mount) mount.removeChild(renderer.domElement);
    };
  }, [progressRef]);
...
function Timeline({ events }) {
  return (
    <ol className="timeline">
      {events.map((e, i) => (
        <li key={i} className="timeline-row reveal" style={{ animationDelay: `${i * 0.06}s` }}>
...
function BlackHoleResearch() {
  ...
  const prologue = [
    ...
    {
      year: "1796", who: "Pierre-Simon Laplace", title: "独立的法语推演",
  ...
  const chapter1 = [
    ...
    {
      year: "1915", who: "Albert Einstein · David Hilbert", title: "广义相对论场方程",
      text: "经过八年挣扎，爱因斯坦在 1915 年 11 月以一组方程把引力转化为时空曲率本身。同月希尔伯特用作用量原理独立给出等价推导。物质告诉时空如何弯曲，时空告诉物质如何运动。",
      math: { lhs: <>G<sub>μν</sub> + Λ g<sub>μν</sub></>, rhs: <><Frac num={<>8π G</>} den={<>c<sup>4</sup></>} /> T<sub>μν</sub></> },
    },
  ];

  const chapter2 = [
    {
      year: "1916", who: "Karl Schwarzschild", title: "第一个精确解",
      text: "在俄国前线的战壕中，史瓦西仅用数周便求出场方程的第一个精确解 —— 描述球对称、不带电、不旋转质量周围的时空。三个月后他病逝于战壕。这个解中藏着事件视界。",
    },
    {
      year: "1916", who: "Hans Reissner", title: "带电黑洞解",
      text: "Reissner 求出带电球对称黑洞的解，1918 年由 Nordström 独立完成。第二个参数 Q 被引入。",
    },
    {
      year: "1924", who: "Arthur Eddington", title: "坐标的真相",
      text: "爱丁顿提出新坐标系，证明史瓦西半径 r = 2GM/c² 处的「奇异性」只是坐标选择不当的假象 —— 那里时空是光滑的，真正的奇点在 r = 0。",
    },
    {
      year: "1932", who: "Georges Lemaître", title: "可穿越的视界",
  ...
  const chapter3 = [
    {
      year: "1931", who: "Subrahmanyan Chandrasekhar", title: "钱德拉塞卡极限",
      text: "19 岁的钱德拉塞卡在前往剑桥的航船上完成计算：电子简并压只能支撑约 1.4 倍太阳质量以下的白矮星。质量超过此值，坍缩不可避免。",
      math: { lhs: <>M<sub>Ch</sub></>, rhs: <>≈ 1.4 M<sub>☉</sub></> },
    },
    {
      year: "1939", who: "Tolman · Oppenheimer · Volkoff", title: "TOV 方程",
      text: "Tolman 与 Oppenheimer-Volkoff 推导了相对论性流体静力学平衡方程，给出中子星的质量上限。压力本身贡献引力 —— 这是相对论独有的反直觉机制。",
      math: {
        lhs: <>d p / d r</>,
        rhs: <>−G[ρ + p/c<sup>2</sup>][m(r) + 4π r<sup>3</sup>p/c<sup>2</sup>] / r<sup>2</sup>(1 − 2Gm/rc<sup>2</sup>)</>,
      },
    },
    {
      year: "1939", who: "Oppenheimer · Snyder", title: "持续坍缩",
    ...
    <div className="bh-root">
      ...
      <header className="masthead">
        <div className="masthead-l">
          <div className="logo-mark"><span className="logo-dot" /></div>
      ...
      {/* ═══ PROLOGUE ═══ */}
      <Section part="序章 · Prologue" title={<>思想的<i>萌芽</i></>} intro="在广义相对论尚未诞生的一百多年前，黑洞的核心思想已经在牛顿引力的框架下浮现。">
        <Timeline events={prologue} />
      </Section>

      {/* ═══ CHAPTER I ═══ */}
      <Section part="第一章 · Chapter I" title={<>时空的<i>革命</i></>} intro="1905 年到 1915 年，爱因斯坦用十年时间把引力从「力」变成了「几何」。这是黑洞研究的根。" accent="cyan">
        ...
        <FeatureEquation
          ...
          equation={<>
            ...
            <span className="big-eq-rhs"><Frac num={<>8π G</>} den={<>c<sup>4</sup></>} /> <span className="ml-half">T<sub>μν</sub></span></span>
          </>}
          notes={[["G", "爱因斯坦张量，时空曲率"], ["Λ", "宇宙学常数"], ["g", "度规张量，定义距离"], ["T", "应力-能量张量，物质与能量"]]}
        />
      </Section>

      {/* ═══ CHAPTER II ═══ */}
      <Section part="第二章 · Chapter II" title={<>第一个<i>解</i></>} intro="1916 年 1 月，俄国前线的战壕中，一封寄给爱因斯坦的信件改变了一切。">
      ...
      {/* ═══ CHAPTER III ═══ */}
      <Section part="第三章 · Chapter III" title={<>恒星的<i>命运</i></>} intro="数学解是一回事，物理实现是另一回事。1930 年代物理学家开始追问：什么样的恒星会坍缩成黑洞？" accent="gold">
        <Timeline events={chapter3} />
      </Section>

      {/* ═══ CHAPTER IV ═══ */}
      <Section part="第四章 · Chapter IV" title={<>黄金<i>时代</i></>} intro="1958 — 1975，约二十年间，黑洞从抽象奇点成为可计算的物理对象。理论物理的一个英雄时代。">
          ...
          equation={<>
            ...
            <span className="big-eq-rhs">
              <span className="triple">
                <span>M</span><span className="comma">,</span>
                <span>J</span><span className="comma">,</span>
                <span>Q</span>
              </span>
            </span>
          </>}
          notes={[["M", "总质量"], ["J", "总角动量"], ["Q", "总电荷"], ["其他", <>所有 multipole moments 由 (M, J, Q) 完全决定</>]]}
        />
      </Section>

      {/* ═══ CHAPTER V ═══ */}
      <Section part="第五章 · Chapter V" title={<>黑洞<i>热力学</i></>} intro="1971 — 1974，三年内黑洞与热力学的对应关系被精确建立。这或许是现代物理学最深刻的发现之一。" accent="cyan">
        ...
        <FeatureEquation
          ...
          equation={<>
            ...
            <span className="big-eq-eq">=</span>
            <span className="big-eq-rhs"><Frac num={<>k<sub>B</sub> c<sup>3</sup> A</>} den={<>4 ℏ G</>} /></span>
          </>}
          notes={[
            ["A", "事件视界面积"],
            ["ℓ_P", <>= √(ℏG/c³)，普朗克长度</>],
        ...
        <FeatureEquation
          ...
          equation={<>
            ...
            <span className="big-eq-eq">=</span>
            <span className="big-eq-rhs"><Frac num={<>ℏ c<sup>3</sup></>} den={<>8π G M k<sub>B</sub></>} /></span>
          </>}
          notes={[
          ...
          <div className="hawking-overlay">
            <div className="hawking-mono mono">
              <div className="hawking-progress-line">
                <div className="hawking-progress-fill" style={{ width: `${hawkingProgress * 100}%` }} />
              </div>
        ...
        <div className="hawking-scroll-content">
          <div className="hawking-step">
            ...
            <p className="hawking-step-text">
              在事件视界之外，量子真空并非真空。虚粒子对不断产生又湮灭，这是量子场论最基本的图景。
            </p>
          </div>
          ...
          <div className="hawking-step">
            ...
            <p className="hawking-step-text">
              霍金在 1974 年的关键洞察是：当虚粒子对恰好诞生于事件视界两侧时，一个粒子可以在外部时空中获得正能量并飞出，另一个则被视界俘获，携带「负能量」落入。
            </p>
            <Eq label="对产生 → 实辐射">
          ...
          <div className="hawking-step">
            ...
            <p className="hawking-step-text">
              落入的负能量降低黑洞质量。从外部观察者看来，黑洞向外发射了一束温度为 T_H 的近热辐射 —— 这就是霍金辐射。视界正在缓慢萎缩。
            </p>
          ...
          <div className="hawking-step">
            ...
            <p className="hawking-step-text">
              注意一个反直觉事实：黑洞质量越小，温度越高。蒸发是一个加速过程 —— 最后阶段以爆炸式的能量喷发结束。这与你的咖啡完全相反。
            </p>
            <Eq>
          ...
          <div className="hawking-step">
            ...
            <p className="hawking-step-text">
              如果辐射真是热的，那么任何关于落入物质的信息都将被抹除 —— 这违反量子力学的幺正性。这就是黑洞信息悖论，至今仍是基础物理最深刻的悬而未决之问题。
            </p>
          </div>
        </div>
      </section>
      ...
      <Section part="第七章 · Chapter VII" title={<>看见<i>不可见者</i></>} intro="从 X 射线到引力波，从恒星轨迹到事件视界 —— 二十一世纪，黑洞终于从理论走入照片。">
        <div className="obs-list">
          {observations.map((o, i) => (
            <div key={i} className="obs-row reveal" style={{ animationDelay: `${i * 0.05}s` }}>
      ...
      {/* ═══ MASS CLASSIFICATION ═══ */}
      <Section part="第八章 · Chapter VIII" title={<>三种<i>质量尺度</i></>} intro="黑洞按质量大致划分为三类。它们的形成机制不同，在宇宙中扮演的角色也截然有别。">
        <div className="mass-grid">
          {masses.map((m, i) => (
            <div key={m.key} className="mass-cell">
              <div className="mass-orb-wrap" style={{ height: `${m.sizeRem * 4 + 1}rem` }}>
                <div className="mass-orb" style={{ width: `${m.sizeRem * 4}rem`, height: `${m.sizeRem * 4}rem` }} />
                <div className="mass-ring" style={{ width: `${m.sizeRem * 4 + 0.6}rem`, height: `${m.sizeRem * 4 + 0.6}rem` }} />
              </div>
              <div className="mass-meta">
                <div className="mono kicker tracked accent-gold">Class {String.fromCharCode(0x2160 + i)}</div>
                <h3 className="mass-name">{m.name}</h3>
                <div className="mass-latin">{m.latin}</div>
                <div className="mass-mass">{m.mass} <span className="mass-unit">{m.unit}</span></div>
      ...
      {/* ═══ EPILOGUE ═══ */}
      <Section part="尾声 · Epilogue" title={<>未尽的<i>边疆</i></>} intro="即便已能「看见」黑洞，关于它最深的问题仍未解答。这五个方向，正定义着未来十年的物理学。" accent="gold">
        <ul className="frontier-list">
          {frontiers.map((f, i) => (
            <li key={i} className="frontier-row reveal" style={{ animationDelay: `${i * 0.05}s` }}>
      ...
      <footer className="footer">
        <div className="footer-quote">
          ...
          <br />to a photograph in 2019 ──
          <br /><span className="accent-ember">the invisible became visible.</span>"
        </div>
...
.bh-root {
  ...
  font-weight: 300;
  min-height: 100vh;
  /* FIX: overflow-x: clip does not create a new scroll container,
     so position:sticky children work correctly.
     overflow-x: hidden would make this element the scroll container
...
.timeline::before {
  ...
  bottom: 2.2rem;
  width: 1px;
  background: linear-gradient(180deg, transparent 0%, rgba(255,122,58,0.35) 5%, rgba(255,122,58,0.35) 95%, transparent 100%);
  z-index: 0;
}
@media (min-width: 768px) {
  .timeline::before { left: 19.5px; top: 3rem; bottom: 3rem; }
}
.timeline-row {
  display: grid;
...
@media (min-width: 768px) {
  .timeline-row { grid-template-columns: 40px 110px 1fr; gap: 2.5rem; padding: 2.5rem 0; }
}
.timeline-dot-col { position: relative; display: flex; justify-content: center; padding-top: 0.5rem; }
.timeline-dot {
  width: 11px; height: 11px;
  border-radius: 50%;
  background: var(--ember);
  box-shadow: 0 0 0 4px var(--void), 0 0 0 5px rgba(255,122,58,0.35), 0 0 18px rgba(255,122,58,0.6);
  flex-shrink: 0;
  z-index: 2;
}
.bh-section:nth-of-type(even) .timeline-dot {
  box-shadow: 0 0 0 4px var(--ink), 0 0 0 5px rgba(255,122,58,0.35), 0 0 18px rgba(255,122,58,0.6);
}
.timeline-year { color: var(--gold); font-size: 0.95rem; padding-top: 0.1rem; font-weight: 500; }
...
.timeline-text { font-size: 0.95rem; line-height: 1.85; color: var(--paper); font-weight: 300; margin-bottom: 1rem; }
...
.feature-eq-caption { color: var(--cyan); margin-bottom: 2rem; }
.feature-eq-display {
  font-family: 'Cormorant Garamond', serif;
  font-style: italic;
...
@media (min-width: 768px) { .feature-eq-notes { grid-template-columns: 1fr 1fr; gap: 1rem 2.5rem; } }
.feature-eq-note {
  display: grid;
...
.feature-eq-sym  { font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 1.1rem; color: var(--cyan); font-weight: 500; }
...
.frac-num, .frac-den { padding: 0 0.35rem; font-size: 0.85em; }
.frac-bar { display: block; width: 100%; height: 1px; background: currentColor; opacity: 0.6; margin: 0.15rem 0; }
...
/* FIX: sticky needs its scroll context to be the viewport (or a real
   scrollable ancestor). We must NOT set overflow on .hawking-section
   or any ancestor up to body that would create a scroll container. */
.hawking-sticky {
  position: sticky;
  top: 0;
  height: 100vh;
...
.eq-rule { position: absolute; top: 0; left: 0; bottom: 0; width: 2px; background: var(--ember); }
.eq-body {
  font-family: 'Cormorant Garamond', serif;
  font-style: italic;
  font-size: 1.3rem;
...
/* ── observations list ── */
.obs-list { border-top: 1px solid var(--ash); }
.obs-row {
  display: grid;
...
.obs-row:hover { background: rgba(255,122,58,0.03); padding-left: 1rem; }
@media (min-width: 768px) {
  .obs-row {
...
.obs-title { font-family: 'Noto Serif SC', serif; font-size: 1.25rem; font-weight: 500; color: var(--pearl); }
.obs-desc  { font-size: 0.95rem; line-height: 1.8; color: var(--paper); font-weight: 300; }
.obs-arrow { color: var(--smoke); transition: all 0.4s ease; justify-self: end; }
.obs-row:hover .obs-arrow { color: var(--ember); transform: translate(2px,-2px); }

/* ── mass classes ── */
.mass-grid { display: grid; grid-template-columns: 1fr; gap: 4rem; align-items: end; padding: 2rem 0; }
@media (min-width: 768px) { .mass-grid { grid-template-columns: 1fr 1fr 1fr; gap: 2rem; } }
.mass-cell { display: flex; flex-direction: column; align-items: center; text-align: center; }
.mass-orb-wrap { position: relative; width: 100%; display: flex; justify-content: center; align-items: center; }
.mass-orb {
  position: relative;
  border-radius: 50%;
  background: radial-gradient(circle at 50% 50%, #000 0%, #000 55%, transparent 75%);
  box-shadow: 0 0 24px rgba(255,122,58,0.25), 0 0 80px rgba(255,122,58,0.12), inset 0 0 8px rgba(255,122,58,0.15);
  transition: all 0.5s ease;
  z-index: 2;
}
.mass-cell:hover .mass-orb {
  box-shadow: 0 0 36px rgba(255,122,58,0.5), 0 0 120px rgba(255,122,58,0.25), inset 0 0 8px rgba(255,122,58,0.25);
}
.mass-ring {
...
.mass-cell:hover .mass-ring { opacity: 0.85; transform: translate(-50%,-50%) scale(1.05); }
.mass-meta   { margin-top: 2rem; }
.mass-name   { margin-top: 0.75rem; font-family: 'Noto Serif SC', serif; font-weight: 500; font-size: 1.4rem; color: var(--pearl); }
.mass-latin  { margin-top: 0.25rem; font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 0.9rem; color: var(--dust); }
.mass-mass   { margin-top: 0.85rem; font-family: 'JetBrains Mono', monospace; font-size: 1.05rem; color: var(--ember); font-weight: 500; }
.mass-unit   { color: var(--paper); }
.mass-origin { margin-top: 0.75rem; font-size: 0.85rem; color: var(--paper); font-weight: 300; }
.mass-axis   { margin-top: 4rem; display: flex; align-items: center; gap: 1rem; }
.mass-axis-line { flex: 1; height: 1px; background: linear-gradient(90deg, var(--ash), rgba(255,122,58,0.4), var(--ember)); }
...
.frontier-row:hover { padding-left: 1rem; background: rgba(255,122,58,0.03); }
@media (min-width: 768px) { .frontier-row { gap: 2.5rem; padding: 2.5rem 0; } }
.frontier-num   { color: var(--gold); font-size: 0.95rem; }
.frontier-title { font-family: 'Noto Serif SC', serif; font-weight: 500; font-size: 1.25rem; color: var(--pearl); }
@media (min-width: 768px) { .frontier-title { font-size: 1.5rem; } }
.frontier-desc  { margin-top: 0.85rem; font-size: 0.95rem; line-height: 1.85; color: var(--paper); font-weight: 300; max-width: 56ch; }
.frontier-arrow { color: var(--ember); opacity: 0; transform: translate(-8px,8px); transition: all 0.4s ease; }
.frontier-row:hover .frontier-arrow { opacity: 1; transform: translate(0,0); }
...
.footer-meta { display: grid; grid-template-columns: 1fr; gap: 2.5rem; margin-top: 4rem; }
@media (min-width: 640px) { .footer-meta { grid-template-columns: 1fr 1fr; } }
.footer-meta-val { margin-top: 0.5rem; color: var(--bone); font-size: 1rem; }
...
@media (min-width: 768px) { .footer-end { flex-direction: row; justify-content: space-between; align-items: center; } }

.inline-block { display: inline-block; }
.mr-1 { margin-right: 0.25rem; }
`;

window.BlackHoleResearch = BlackHoleResearch;